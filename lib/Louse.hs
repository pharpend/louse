    ...
    CommentTree
      (mconcat (do Node (auth,commentTxt) subcomments <- forest_
                   let subcommentTree = fromForest subcomments
                       commentHash = sha1 (unCommentText commentTxt)
...
instance VerifyRec CommentTree where
  verifyRec (CommentTree unct) =
    do (key,cmt@(Comment _ txt subtree)) <- H.toList unct
       let goodKey = sha1 (unCommentText txt)
           subTreeMessages = verifyRec subtree
       if (key == goodKey)
          then subTreeMessages
